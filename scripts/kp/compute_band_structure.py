...
def compute_spectrum(kx, ky, parameters, band_number, old=False):
    ...
    energies = np.empty((len(ky), parameters.site_number*band_number))
    # energies = np.empty((len(kx), 8))

    for i in range(len(ky)):
        if old:
            h = build_hamiltonian_old(ky[i], -ky[i], parameters, band_number)
        else:
            h = build_hamiltonian(ky[i], -ky[i], parameters, band_number)
...
layer_1 = LayerParameters(6, ['hgte', 'cdte'], [0.32, 0.68])

layer_2 = LayerParameters(4, ['hgte'], [1.0])

layer_3 = LayerParameters(6, ['hgte', 'cdte'], [0.32, 0.68])
...
energies_old = compute_spectrum(kx, ky, h_par, BAND_NUMBER, True)
