...
def find_trace_theta_cvxpy(l: np.ndarray, c_phi: np.ndarray, c_3phi: np.ndarray, s_phi: np.ndarray, s_3phi: np.ndarray,
               theta_alpha: float, theta_beta3: float, theta_beta1: float) -> float:
    """Find the trace of the matrix R_tot^2
...
def compute_traces_theta_cvxpy(index, l, c_phi, c_3phi, s_phi, s_3phi, theta_alpha, theta_beta1, theta_beta3, N_orbit):
    """Compute the trace of the evolution operator for different trajectories
    ...
    for n in prange(N_orbit//1000):
        for i in range(1000):
            ...
            if traj_id >= N_orbit:
                break
            begin, end = index[traj_id]
            T_a = find_trace_theta_cvxpy(l[begin:end], c_phi[begin:end], c_3phi[begin:end], s_phi[begin:end], s_3phi[begin:end],
                                         theta_alpha, theta_beta3, theta_beta1)
            T[traj_id] = T_a
