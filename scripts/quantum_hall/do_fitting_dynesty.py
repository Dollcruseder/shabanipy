...
from shabanipy.quantum_hall.conversion\
    import (convert_lock_in_meas_to_diff_res, GEOMETRIC_FACTORS,
            ...
            fermi_velocity_from_kf, diffusion_constant_from_mobility_density)
...
from shabanipy.quantum_hall.wal.fitting_dynesty import fitting_dynesty
import h5py
...
D = diffusion_constant_from_mobility_density(mobility, density, mass)

field, res['xx'] = symmetrize_wal_data(field, res['xx'], SYM_METHOD)

fitting_dynesty(field[:], res['xx'][:], WAL_REFERENCE_FIELD, WAL_MAX_FIELD,
