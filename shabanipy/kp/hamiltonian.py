...
def build_hamiltonian(kx, ky, parameters, band_number):
    ...
    for i in range(parameters.site_number):
        ...
        if bn >= 6:
            ...
            h[ind+2, ind+4] = -0.5*bia*(kx - 1j*ky)
            h[ind+2, ind+5] = R
            h[ind+3, ind+4] = R.conjugate()
            h[ind+3, ind+5] = -0.5*bia*(kx + 1j*ky)
    ...
    for i in range(parameters.site_number-1):
        ...
        c = 1j*b*0.5*i_step*(kappa[i+1] - kap)*(kx - 1j*ky)
        ...
        if bn >= 6:
            ...
            # complex conjugated
            sb = 1j*b*sqrt3*0.5*i_step*(g3 + gamma3[i+1] - kappa[i+1] + kap)
            sb_m = sb*(kx - 1j*ky)
            sb_p = sb*(kx + 1j*ky)
            sbd = 1j*b*sqrt3*0.5*i_step*(g3 + gamma3[i+1] + kappa[i+1] - kap)
            sbd_m = sbd*(kx + 1j*ky)  # mind the sign change
            sbd_p = sbd*(kx - 1j*ky)  # mind the sign change

            # Additional terms needed in the 6 bands model on the previously
            # filled lines
            h[ind+2, ind+bn+4] = - sbd_m
            h[ind+2, ind+bn+5] = 1j*bia*i_step*0.5
            h[ind+3, ind+bn+4] = - 1j*bia*i_step*0.5
            h[ind+3, ind+bn+5] = sbd_p
            ...
            # Fifth line: gamma 8 +3/2
            h[ind+4, ind+bn+2] = - sb_m  # Match docs and fortran
            ...
            h[ind+5, ind+bn+3] = sb_p  # Match docs and fortran
            ...
            if bn == 8:
                ...
                st = 1j*b*sqrt3*0.5*i_step*(g3 + gamma3[i+1] +
                                            i_3*(kappa[i+1] - kap))
                st_m = st*(kx - 1j*ky)
                st_p = st*(kx + 1j*ky)
                std = 1j*b*sqrt3*0.5*i_step*(g3 + gamma3[i+1] -
                                             i_3*(kappa[i+1] - kap))
                std_m = std*(kx + 1j*ky)  # mind the sign change
                std_p = std*(kx - 1j*ky)  # mind the sign change
                ...
                h[ind+4, ind+bn+6] = i_sqrt2 * sb_m
                h[ind+5, ind+bn+7] = i_sqrt2 * sb_p
                ...
                h[ind+7, ind+bn+6] = c.conjugate()
                h[ind+7, ind+bn+7] = u1

                # Symmetrize
                local_hermit(h, ind+6, ind+bn)
                local_hermit(h, ind+6, ind+bn+2)
                local_hermit(h, ind+6, ind+bn+3)
                local_hermit(h, ind+6, ind+bn+4)
